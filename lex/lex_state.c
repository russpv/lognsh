...
static inline bool	_handle_eof_state(t_lex *lexer)
{
	...
	{
		debug_print("Transitioning to DONE state\n");
		lexer->state = DONE;
		return (true);
	}
	return (false);
...
static inline bool	_handle_quote_state(t_lex *lexer)
{
	...
	{
		debug_print("Transitioning to IN_DOUBLE_QUOTES state\n");
	...
	{
		debug_print("Transitioning to IN_SINGLE_QUOTES state\n");
...
static inline int	_handle_heredoc_state(t_lex *lexer)
{
	...
	{
		debug_print("Transitioning to IN_HEREDOC state\n");
		lexer->ptr += 2;
