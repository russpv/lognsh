...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/05 09:30:06 by dayeo             #+#    #+#             */
/*   Updated: 2025/04/05 21:30:09 by dayeo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	debug_print_tail_info(struct s_merge *m)
{
	...
	if (NULL != (*m->end)->next && NULL != m->r->next)
	{
		debug_print(BLUE "(tail now at: %s | l at: %s, l->next: %s, mid: %s, \
			mid->next: %s "
			"| r at: %s, r->next: %s, end: %s, \
			end->next: %s ""| beg: %s) \n" RESET, (*m->tail)->content, \
			m->l->content, m->l->next->content, m->mid->content, \
			m->mid->next->content, m->r->content, m->r->next->content, \
			(*m->end)->content, (*m->end)->next->content, (*m->beg)->content);
	}
	else if (NULL == (*m->end)->next && NULL != m->r->next)
	{
		debug_print(BLUE "(tail now at: %s | l at: %s, l->next: %s, mid: %s, \
			mid->next: %s ""| r at: %s, r->next: %s, end: %s, end->next: (end)) "
                      "| beg: %s) \n" RESET,
                    (*m->tail)->content, m->l->content, m->l->next->content, \
                    m->mid->content, m->mid->next->content,
                    m->r->content, m->r->next->content, (*m->end)->content,
                    (*m->beg)->content);
    }
    else if (NULL == (*m->end)->next) {
        debug_print(BLUE "(tail now at: %s | l at: %s, \
        	l->next: %s, mid: %s, mid->next: %s "
                      "| r at: (end), r->next: (end), end: %s, \
                      	end->next: (end)) "
                      "| beg: %s) \n" RESET,
                    (*m->tail)->content, m->l->content, \
                    	m->l->next->content, m->mid->content, \
                    m->mid->next->content,
                    (*m->end)->content,
                    (*m->beg)->content);
    }
    else if (NULL == (*m->tail)) {
        debug_print(BLUE "(tail now at: end | l at: %s, \
        	l->next: %s, mid: %s, mid->next: %s "
                      "| r at: (end), r->next: (end), end: %s, \
                      	end->next: (end)) "
                      "| beg: %s) \n" RESET,
                    m->l->content, m->l->next->content, \
                    	m->mid->content, m->mid->next->content,
                    (*m->end)->content,
                    (*m->beg)->content);
    }
...
/*void	print_diagnostics_end(t_list **lst, struct s_merge *m)
{
	...
	if (*m->tail && (*m->tail)->next)
		debug_print( "|beg->next...tail->next \
			: %s->%s...%s->%s|\n", (*m->beg)->content, \
		(*m->beg)->next->content, (*m->tail)->content, \
			(*m->tail)->next->content);
	ft_lstprint_betw(*m->beg, *m->tail);
	ft_lstprint_betw(*m->beg, *m->end);
	ft_lstprint(*lst);
	debug_print( "RETURNING head: \
...
void	print_prev_post(struct s_merge *m)
{
	if (NULL != (*m->tail)->prev && NULL != (*m->tail)->prev->prev)
		debug_print( MAGENTA" prev now \
			linked %s<-%s->%s"RESET") -> \n", \
			(*m->tail)->prev->prev->content, \
			(*m->tail)->prev->content, \
...
{
	if (NULL != (*m->tail)->prev)
	debug_print(BLUE" -reassign prev %s \
		to:%s- "RESET, (*m->tail)->prev->content, \
		m->tmp->content);
	else
...
void	print_swap_post(t_list **lst, struct s_merge *m)
{
	if ((*m->tail) == *lst){
		...
		debug_print( CYAN"%s "GREEN"(prev: %s, \
			next: END), -> \n"RESET, \
			(*m->tail)->content, (*m->tail)->prev->content);
}
...
void	print_tailnext_post(struct s_merge *m)
{
	...
	if (*m->tail && (*m->end)->next && m->tmp2)
		debug_print( BLUE"status: end->next %s, \
			tail:%s tail->next: %s, \
			tmp2:%s\n"RESET, (*m->end)->next->content, (*m->tail)->content,\
		(*m->tail)->next->content, (m->tmp2)->content);
	if (*m->tail)
		debug_print( "%s (remainder, \
			"MAGENTA"prev:%s->%s) -> "RESET, (*m->tail)->content, \
			(*m->tail)->prev->content, (*m->tail)->prev->next->content);
}
