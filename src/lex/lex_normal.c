 ...
 */
static t_tok	*_match_word_or_name_or_exception(t_lex *lexer)
{
 ...
 */
static t_tok	*_match_reserved_token(t_lex *lexer)
{
	t_tok	*res;
...
static t_tok	*_match_normal(t_lex *lexer)
{
	...
	if (true == is_transition_delim(*lexer->ptr, (lexer->ptr + 1)))
		return (NULL);
	res = _match_reserved_token(lexer);
	if (res)
		return (res);
	res = _match_word_or_name_or_exception(lexer);
	if (res)
		return (res);
