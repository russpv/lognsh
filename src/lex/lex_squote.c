...
{
	debug_print("_load_buf... ptr:_%c_\n", *lexer->ptr);
	if ((unsigned char)OP_SQUOTE == *lexer->ptr)
	{
		debug_print("tokenize_single_quotes: found closing quote\n");
		return (1);
	}
	else
	{
		if (*buf_idx < LEX_BUFSZ - 1)
		{
			...
			{
				debug_print("tokenize_single_quotes: found null before closing quote\n");
				return (1);
			}
...
int	tokenize_single_quotes(t_lex *lexer)
{
	...
	debug_print("tokenize_single_quotes: created token\n");
	if (NULL == token)
		return (1);
