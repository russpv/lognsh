...
// Assumes *(l->ptr + offset) is valid
static unsigned char	_is_valid_next(t_lex *l, int offset)
{
	if (!l || !l->ptr)
		return (0);
	if (offset < 0 || l->ptr[offset + 1] == '\0')
        return (0);
...
bool	is_dollar_delim(t_lex *l)
{
	if (!l || !l->ptr || !l->ptr[0] || !l->ptr[1])
		return (false);
	return ('$' == l->ptr[0] && (is_varnamechar(l->ptr[1]) || '?' == l->ptr[1]));
}
...
bool	is_normal_delim(t_lex *lexer, int offset)
{
	...
	if (!_is_valid_offset(lexer, offset))
		return (true);
	c = *lexer->ptr + offset;
	next = _is_valid_next(lexer, offset);
	if (c && next)
