...
#define DBGMSG_GOTARGS _MOD_ ": %s - arg: %s, lst:%p\n"
#define DBGMSG_MATCHES _MOD_ ": %s found %d matches, 1st: %s\n"
...
int	p_do_globbing_toks(t_mem_mgr *mgr, t_list **lst_node, void *lst_c)
{
	...
	if (!tok_get_raw(tok))
		return (ERR_ARGS);
	debug_print(DBGMSG_GOTARGS, __FUNCTION__, tok_get_raw(tok), (void *)lst_node);
...
int	p_do_globbing_args(t_mem_mgr *mgr, t_list **lst_node, void *lst_c)
{
	...
	if (!arg->raw)
		return (ERR_ARGS);
	debug_print(DBGMSG_GOTARGS, __FUNCTION__, arg->raw, (void *)lst_node);
