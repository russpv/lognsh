...
#define DEBUGMSG_REDIR_NODE "Parser: p_do_globbing_redirs got redir type: %d fn:%s doc:\
		%s do_glob:%d\n"
#define DEBUGMSG_REDIR_GLOB "Parser: p_do_globbing_redirs found:%s...\n"
#define DEBUGMSG_GOTARGS "Parser: p_do_globbing - arg: %s, lst:%p\n"
#define DEBUGMSG_MATCHES "Parser: p_do_globbing found %d matches, 1st: %s\n"
#define DEBUGMSG_GOTPATTERN "Parser: match_glob got pattern: %s\n"
#define DEBUGMSG_GLOBMATCH "Parser: match_glob matched: %s\n"
...
static t_list	*_match_glob(const char *pattern)
{
	...
	lst = NULL;
	debug_print(DEBUGMSG_GOTPATTERN, pattern);
	dir = opendir(".");
	if (!dir)
		return (perror(ERRMSG_OPENDIR), NULL);
	res = readdir(dir);
	while (res)
	{
		if (true == _matches_pattern(res->d_name, pattern))
		{
			debug_print(DEBUGMSG_GLOBMATCH, res->d_name);
...
int	p_do_globbing_redirs(void *c)
{
	...
	debug_print(DEBUGMSG_REDIR_NODE, r->type, r->filename,\
	...
	if (true == r->do_globbing)
	{
		...
		if (lst)
		{
			debug_print(DEBUGMSG_REDIR_GLOB,(char *)lst->content);
			if (ft_lstsize(lst) > 1)
...
/* DO GLOBBING
 *
 * Passed to t_list *redirs 
 * reverse iterator to handle insertions. 
...
int	p_do_globbing(t_list **lst_node, void *lst_c)
{
	...
	debug_print(DEBUGMSG_GOTARGS, content->raw, (void *)lst_node);
	if (true == content->do_globbing)
	{
		lst = _match_glob(content->raw);
		if (lst)
		{
			debug_print(DEBUGMSG_MATCHES,ft_lstsize(lst), lst->content);
