...
#define DBGMSG_REDIR_NODE \
	"%s: p_do_globbing_redirs got redir type: %d fn:%s doc:\
		%s do_glob:%d\n"
#define DBGMSG_REDIR_GLOB  "%s: p_do_globbing_redirs found:%s...\n"
...
int	p_do_globbing_redirs(t_mem_mgr *m, void *c)
{
	...
	r = (t_redir_data *)c;
	dprint(DBGMSG_REDIR_NODE, _MOD_, r->type, r->filename, r->heredoc_body,
		r->do_globbing);
	if (true == r->do_globbing)
	{
		...
		if (r->lst_glob)
		{
			dprint(DBGMSG_REDIR_GLOB, _MOD_, (char *)r->lst_glob->content);
