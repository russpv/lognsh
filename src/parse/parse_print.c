...
static void	_print_cmd_helper(t_ast_node *ast)
{
	t_arg_data	*arg;
	t_list		*arg_node;
...
static void	_print_cmd(t_ast_node *ast)
{
	...
	if (ast->data.cmd.redc > 0)
	{
		...
		while (redir_node != NULL)
		{
			...
			cprintf(CYAN, "%d|%s %s g_%d exp_%d", redir->type,
				redir->symbol, redir->filename, redir->do_globbing,
				redir->do_expansion);
